...
export function useAsyncState<T>(
  initialResolver?: () => Promise<T>
): [AsyncProperty<T>, (resolver: () => Promise<T>) => void, () => void] {
  ...
  const [captureContainer] = preactHooks.useState<{
    previousCapture: { setResult: preactHooks.StateUpdater<AsyncProperty<T>> };
  }>({ previousCapture: { setResult: () => {} } });
  let firstRun = false;
  const [result, setResult] = preactHooks.useState<AsyncProperty<T>>(() => {
    firstRun = true;
